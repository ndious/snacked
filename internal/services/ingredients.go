...
func (is IngredientService) CreateIngredient(name string, unit models.Units) (ingredient models.Ingredient, err error) {
	query := `
    ...
    RETURNING id;
    `
	err = is.db.QueryRow(query, name, unit).Scan(&ingredient.ID)
